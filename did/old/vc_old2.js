...
async function generateVC() {
    ...
    for (const [key, value] of Object.entries(attributes)) {
        if (key !== 'id') {
            ...
            // Poseidon 해시 사용 (normalize 없이 바로 사용)
            const hashedValue = poseidon([valueBigInt]); // F.normalize 제거
    ...
    // const signature = eddsa.signPoseidon(issuerPrivateKey, [merkleRootField]);
    // BigInt -> Uint8Array 변환
    ...
    const inputs = {
        ...
        oldValue: oldValue ? oldValue.toString() : '0',
        isOld0: isOld0 ? 1 : 0,
    ...
    // 입력 데이터를 JSON 파일로 저장
    fs.writeFileSync('./inputs.json', JSON.stringify(inputs, null, 2), 'utf8');
}
